...
const SCATTER = [
  { x: -220, y: -60, rotZ: -10, rotY: -4 },
  { x: 60, y: -140, rotZ: 8, rotY: 4 },
  { x: -80, y: 110, rotZ: -8, rotY: -3 },
  { x: 200, y: 30, rotZ: 5, rotY: 3 },
];
...
export default function SwiperSection() {
  ...
  useGSAP(() => {
    const raf = requestAnimationFrame(() => {
      ...
      slides.forEach((slide, i) => {
        const p = SCATTER[i] ?? { x: 0, y: 0, rotZ: 0, rotY: 0 };
        gsap.set(slide, {
          x: p.x,
          y: p.y,
          rotationZ: p.rotZ,
          rotationY: p.rotY,
          zIndex: SCATTER.length - i,
        ...
        onEnter: () => {
          slides.forEach((slide, i) => {
            gsap.to(slide, {
              ...
              force3D: true,
              onComplete: () => {
                if (i === slides.length - 1 && swiperRef.current) {
  ...
  return (
    ...
    >
      <div ref={swiperWrapRef} className="w-full px-[3.62vw] h-full h-full">
        <Swiper
        ...
        >
          ...
          {/* pagiation  */}
          <div className="flex items-center justify-center gap-[13px] absolute left-1/2 bottom-[-9.26vh] select-none w-fit mx-auto z-20">

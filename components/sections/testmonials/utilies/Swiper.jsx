...
export default function SwiperSection() {
  ...
  useGSAP(() => {
    const raf = requestAnimationFrame(() => {
      ...
      // Clicking ANY card assembles the whole deck simultaneously
      const handleClick = () => {
        if (assembledRef.current) return;
        assembledRef.current = true;

        // Remove listeners immediately so clicks don't re-fire mid-animation
        ...
        gsap.to(slides, {
          ...
          duration: 2.8,
          ease: "expo.out",
          stagger: 0,       // all cards move at the exact same time
          force3D: true,
          onComplete: () => {
            if (swiperRef.current) {
